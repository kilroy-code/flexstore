...
const localStorage = new Persist({collectionName: 'local', collectionType: 'Storage'});
...
function killRendevous(rendevousTag) { // ... if not completed within timeout. Returns clearable timer.
  return setTimeout(() => {
...
router.get('/signal/offer/:rendevousTag', async (req, res, next) => {
  const {rendevousTag} = req.params; // Get the pending offer.
  let stashedData = rendevous[rendevousTag];
  if (!stashedData) { // If we're first, await a promise that will be resolved by the peer.
    stashedData = await new Promise(resolve => rendevous[rendevousTag] = resolve);
  }
  const {offer} = stashedData;
  res.send(offer);
});
router.post('/signal/:role/:rendevousTag', async (req, res, next) => {
  ...
  case 'answer': // Stash our offer and await a promise for an answer.
    const peerResolver = rendevous[rendevousTag] || (() => null); // Peer might already be waiting.
    const answer = await new Promise(resolve => peerResolver(rendevous[rendevousTag] = {resolve, offer:body}));
    return res.send(answer);
  case 'offer': // Previous GET got the offer. Now we resolve the promise with our answer.
    const {resolve} = rendevous[rendevousTag];
    delete rendevous[rendevousTag];
    resolve(body);
    return res.send({});
  default:
...
// - The kind/collectionName/uuid are part of our uniform routing, which is designed to allow several different
//   connections to explicitly connect to the same url with separate connections. We currently handle this at
//   both ends by multiplexing, so there is no need. But we retain the convention in case we go back to that.
// /:ignoredKind/:ignoredCollectionName/:ignoredUuid
//     if (offer) return res.sendStatus(409); // "conflict" because already in use
//    if (!offer) return res.sendStatus(404); // FIXME: should wait for an offer.
    /*
    const timeout = setTimeout(() => {
      delete rendevous[rendevousTag];
      return res.senStatus(408); // timeout. Client can try again.
      }, rendevous_timeout);
      */
