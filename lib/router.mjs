...
*/
const massPushIntervalMS = 20 * 60 * 60e3; // 20 minutes, in milliseconds.
let massPushTimer = null;
function noteActivity() { // On there being any actvity.
  console.log('activity recent actvity:', !!massPushTimer, new Date());
  if (massPushTimer) return;
  massPushTimer = setTimeout(() => { massPush(); massPushTimer = null; }, massPushIntervalMS);
  massPush();
}
async function massPush() {
  ...
  for (let guid of await subscriptions.list()) {
    const isOnline = testCollection.synchronizers.get(guid);
    console.log('massPush', guid, !!isOnline);
