...
async function massPush() {
  ...
  console.log('massPush', new Date(), guids);
...
const collections = { ImmutableCollection: {}, MutableCollection: {}, VersionedCollection: {}, StateCollection: {}};
...
// - which peer will make the "offer" and which will be the "answer". ("lead" and "follow" can be in either order, but there must be one of each)
// - at what time they should connect
const rendevous = {};
const rendevous_timeout = 60e3;
async function supplyAndRespond(req, res, seeking, supplying) {
  // rendevous[rendevousTag] is an object by which we exchange data:
  // The offer and answer properties are either a array of signals if it arrives first, else a Promise resolver.
  //
  // Three cases:
  // POST answer: Supply offer; read or wait for answer; and respond with answer.
  // GET offer: Supply nothing; read or wait for offer; and respond with offer.
  // POST offer: Supply answer; ignore offer that is there-or-resoved; respond empty.
  //
  // Only two possible orders of requests:
  // a. Post4answer sets offer and awaits answer;
  //    Get4offer responds w/offer;
  //    Post4offer resolves answer and responds empty; (Post4answer responds w/answer)
  // b. Get4offer awaits offer;
  //    Post4answer resolves offer and awaits answer; (Get4offer responds w/offer)
  //    Post4offer resolves answer and responds empty; (Post4answer responds answer)

  const {method, params, body} = req;
  const {rendevousTag} = params;
  let data = rendevous[rendevousTag];
  let theirSignals;

  if (!data) { // Ensure a place to put things, with a timeout.
    data = rendevous[rendevousTag] = {timeout: setTimeout(() => { // On expiration, clean up.
      // If offer and answer are resolvers, call them with no args.
      data.offer?.call && data.offer();
      data.answer?.call && data.answer();
      delete rendevous[rendevousTag];
    }, rendevous_timeout)};
  }

  // If we have signals, supply them: either directly or by resolving the other guy's promise.
  if (supplying) {
    const resolve = data[supplying];
    if (resolve?.call) resolve(body);
    else data[supplying] = body;
  }

  // Now get or wait for what we seek, if any.
  if (seeking) {
    theirSignals = data[seeking];
    if (!theirSignals) {
      theirSignals = await new Promise(resolve => data[seeking] = resolve);
    }
  }
  if (!seeking) return res.send({});
  return theirSignals ? res.send(theirSignals) : res.sendStatus(408);
}
...
router.post('/signal/:role/:rendevousTag', async (req, res, next) => {
  ...
  case 'offer': // Previous GET got the offer. This followup seeks nothing back but supplies the answer that the other side is waiting for.
    await supplyAndRespond(req, res, null, 'answer');
    // In all valid orderings, that response is always the last thing, so we can clean up now.
    const rendevousTag = req.params.rendevousTag;
    clearTimeout(rendevous[rendevousTag].timeout);
    delete rendevous[rendevousTag];
    return null;
  default:
