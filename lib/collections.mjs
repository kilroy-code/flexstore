...
export class Collection extends EventTarget {
  ...
  constructor({name, services = [], preserveDeletions = !!services.length, persistenceClass = Collection.Persist, debug}) {
    ...
    this.persist = persistenceClass.then(kind => new kind({collection: this}));
  }
  ...
  push(operation, tag, signature, excludeSynchronizer = null) { // Push to all connected synchronizers, excluding the specified one.
    return Promise.all(this.synchronizers.values().map(synchronizer => (excludeSynchronizer !== synchronizer) && synchronizer.push(operation, tag, signature)));
  }
...
export class VersionedCollection extends MutableCollection {
  ...
  async remove(options = {}) { // Add an empty verion or remove all versions, depending on this.preserveDeletions.
    ...
    if (this.preserveDeletions) { // Create a timestamp => version with an empty payload. Otherwise merging with earlier data will bring it back!
      ...
      await (await this.persist).delete(tag, await Collection.sign('', signingOptions));
    }
    await this.deleteTag(tag);
    return tag;
  }
...
Credentials.synchronize = (...services) => { // TODO: rename this to connect, and definee disconnect, too.
...
Credentials.getUserDeviceSecret = (tag, promptString) => {
  ...
  return prompt(promptString); // Apps should be designed to never hit this.
};
...
export { Credentials };
// The flexstore module is synchronously loadable. However, we don't know which Persistence model to load
// unless we have a build step with conditional module selection, or we use dynamic import.
// But we don't want to hold up the loading to await the dynamic import.
// Instead, we set Collection.Persist once the selected module is loaded, and we resolve 'ready' in case
// any clients need to await the result.
// This mechanism ALSO allows applications to plug in their own definitions for Collection.Persist.
/*
export let ready = new Promise(resolve => {
  import((typeof window === "undefined") ? './persist-fs.mjs' : './persist-indexeddb.mjs')
    .then(module => {
      if (!Collection.Persist) Collection.Persist = module.default; // Don't set it if the application already has done so.
