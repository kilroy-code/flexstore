...
export class Collection extends EventTarget {
  ...
  _canonicalizeOptions(objectOrString = {}) { // Extend _canonicalizeOptions1 to support:
    ...
    if ([true, 'team', 'owner'].includes(encryption)) encryption = team || owner;
...
export class VersionedCollection extends MutableCollection {
  ...
  async store(data, tagOrOptions = {}) {
    ...
    if (!tag) {
      if (ArrayBuffer.isView(data)) tag = await this.hash({payload: data});
      else if (typeof(data) === 'string') tag = await this.hash({text: data});
      else tag = await this.hash({text: JSON.stringify(data)});
    }
  ...
  async mergeSignatures(tag, validation, signature) {
    ...
    if (!await this.sign('anything', asOwner).catch(() => false)) { // We don't have access.
      return await this.sign(combined, asOther); // Just answer the combined list to be persisted.
    }
    ...
    return await this.sign([ancestor], asOwner);
  }
