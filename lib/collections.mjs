...
export class VersionedCollection extends MutableCollection {
  ...
  getOwner(protectedHeader) { // Used in checkOwner.
    const {group, owner} = protectedHeader;
    return group || owner ||super.getOwner(protectedHeader);
  }
  generateOwnerOptions(protectedHeader) { // Generate two sets of signing options: one for owner to use, and one for others
    const {group, owner, iss, kid} = protectedHeader;
    const tags = [Credentials.author];
    if (group) return [{team: group},             {group, tags}];
    if (owner) return [{team: '', member: owner}, {owner, tags}];        // check before iss
    if (iss)   return [{team: iss},               {group: iss, tags}];
    else       return [{team: '', member: kid},   {owner: kid, tags}];
  }
  ...
  async mergeSignatures(tag, validation, signature) {
    ...
    if (!await this.sign('anything', asOwner).catch(() => false)) { // We don't have access.
      return this.sign(combined, asOther); // Just answer the combined list to be persisted.
    }
	...
	// TODO: test that we are propogating encryption propertly.
	const encryption = verified.protectedHeader.encryption || '';
	const signingOptions = {ant: ancestor, iat: verified.protectedHeader.iat, encryption, ...asOwner};
	...
	const reverified = await this.versions.constructor.verify(stateSignature, {member: null});
	ancestor = await this.versions.hash(reverified);
	await this.versions.put(ancestor, stateSignature, verified.synchronizer);
      }
    }
    return this.sign([ancestor], asOwner);
  }
