...
import Persister from '#persist'; // See Persist comments, below.
...
export class Collection extends EventTarget {
  ...
  // This version for nodeJS or browsers, without a build step:
  //   static Persist = import((typeof window === "undefined") ? './persist-fs.mjs' : './persist-indexeddb.mjs').then(module => module.default);
  // When using a builder such as rollup, we can "subpath imports", which are simpler. (See package.json for definition of #persist.)
  static Persist = Persister;

  constructor({name, label = name, services = [], preserveDeletions = !!services.length, persistenceClass = Collection.Persist, dbVersion = storageVersion, debug = false,
    ...
    if (persistenceClass.then) this.persistenceStore = persistenceClass.then(kind => new kind({collection: this}));
    else this.persistenceStore = new persistenceClass({collection: this});
  }
  async close() {
