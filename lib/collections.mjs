...
export class Collection extends EventTarget {
  ...
  constructor({name, label = name, services = [], preserveDeletions = !!services.length, persistenceClass = StorageLocal, dbVersion = storageVersion, debug = false,
    ...
    const persistenceOptions = {name: this.fullLabel, debug: debug};
  ...
  async validateForWriting(tag, signature, operationLabel, synchronizer, requireTag = false) {
    ...
    this.log('emit', {tag, verified, antecedent, synchronize, existingVerified, disallowed});
    this.emit(verified);
    return verified;
  }
  // fixme next 2
