...
export class Collection extends EventTarget {
  ...
  async put(tag, signature, synchronizer = null, mergeAuthorOverride = null) { // Put the raw signature locally and on the specified services.
    ...
    //fixme restore this.log('put', {tag: validation?.tag || tag, synchronizer: synchronizer?.label, text: validation?.text});
  ...
  async disallowWrite(tag, existing, proposed, verified) { // Promise a reason string to disallow, or null if write is allowed.
    ...
    return this.checkOwner(existing, proposed, verified) ??
      this.checkDate(existing, proposed) ??
      this.checkTag(verified) ??
      null;
  }
  async disallowDelete(tag, existing, proposed, verified) { // Deletion typically latches.
    ...
    if (this.getOwner(existing, verified.existing) !== this.getOwner(proposed, verified)) return 'not owner'; // Can't delete someon else's stuff.
    return null; // Deleting trumps data, regardless of timestamp.
  }
  ...
  checkSomething(reason, boolean) {
    return boolean ? reason : null;
  }
  checkOwner(existing, proposed, verified) {// Does proposed owner match the existing?
    return this.checkSomething('not owner', this.getOwner(existing, verified.existing) !== this.getOwner(proposed, verified));
  }
  ...
  async validateForWriting(tag, signature, operationLabel, synchronizer, requireTag = false) { // TODO: Optionals should be keyword.
    ...
    //fixme restore this.log('validateForWriting', {tag, operationLabel, requireTag, fromSynchronizer:!!synchronizer, signature, verified, antecedent, synchronize, existingVerified, disallowed});
...
export class ImmutableCollection extends Collection {
  ...
  async checkTag(verified) { // If the tag doesn't match the data, silently use the existing tag, else fail hard.
    return this.checkSomething(verified.existing ? '' : 'wrong tag', verified.tag !== await this.hash(verified));
  }
}
...
export class StateCollection extends ImmutableCollection {
  ...
  hashablePayload(validation) { // Include ant || iat.
    ...
    return super.hashablePayload(validation) + (ant || iat);
  }
  checkDate() { // always ok
    return null;
  }
  ...
  async commonStateAndMessages(stateTags) {
    ...
    const key = verified => { // By which to dedupe state records.
      const {ant, kid, act, iat} = verified.protectedHeader;
...
export class VersionedCollection extends MutableCollection {
  ...
  static taglessCounter = 0; // If a user stores lots in parallel, they could be at the same timestamp, so count.
  async store(data, tagOrOptions = {}) {
    ...
    tag ||= await this.hash({text: (options.author || Credentials.author) + this.constructor.taglessCounter++});
  ...
  // checkOwner(existing, proposed, verified) {
  //   if (verified.sychronizer) return null; // fixme
  //   return super.checkOwner(existing, proposed, verified);
  // }
  ...
  async mergeSignatures(tag, validation, signature, mergeAuthorOverride) {
    ...
    this.log('mergeSignatures', {existing, states});
    ...
    this.log('mergeSignatures', {existing, states, commonAncestor, versionsToReplay});
    ...
    this.log('mergeSignatures', {claims, owner, signingAs});
    ...
    this.log('mergeSignatures replaying', {state, versionsToReplay, times: versionsToReplay.map(v => v.protectedHeader.iat)});
    for (let verified of versionsToReplay) {
      ...
      state = await this.versions.store(verified.json || verified.text || verified.payload,
					{ant: state, time: verified.protectedHeader.iat, ...signingAs}); // FIXME: preserve encryption of verified
