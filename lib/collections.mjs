...
export class Collection extends EventTarget {
  ...
  static async verify(signature, fixme = {}) {
    ...
    const verified =  await Credentials.verify(signature, fixme);
    return verified;
  }
  ...
  get fixme() {
    if (this.name === 'Team') return {member: null, notBefore: null};
    return {};
  }
  async getVerified(tag, skipSync = false) { // synchronize, get, and verify (but without decrypt)
    ...
    return Collection.verify(signature, this.fixme);
  }
  ...
  async validateForWriting(tag, signature, operationLabel, synchronizer, requireTag = false) {
    ...
    const verified = await Collection.verify(signature, this.fixme);
  ...
  get synchronized() { // promise to resolve when synchronization is complete.
    return Promise.all(this.synchronizers.keys().map(s => s.completedSynchronization));
  }
