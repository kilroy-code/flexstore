...
export class Collection extends EventTarget {
  ...
  async disconnect(...services) { // Shut down the specified services.
    ...
    for (let service of services) {
      ...
      if (!synchronizer) {
	console.warn(`${this.label} (fixme ${this.serviceKey}) does not have a service named '${service}' to disconnect.`);
	continue;
      }
...
export class VersionedCollection extends MutableCollection {
  ...
  constructor({services = [], ...rest} = {}) {
    ...
    this.versions = new VersionCollection({...rest, debug: true}); // Same collection name, but different type.
