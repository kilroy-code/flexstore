...
export class Collection extends EventTarget {
  ...
  async subjectMatch(verified) { // Promises true IFF claimed 'sub' matches hash of the contents.
    return verified.protectedHeader.sub === await this.constructor.hash(verified);
  }
...
export class StateCollection extends ImmutableCollection {
  // In addition to whatever other properties are stored, a state payload has a message property that produced this state
  // when applied to an antecedent state. The antecedent appears twice in the signature:
  // - antecedent is a property of the payload, which makes protectedHeader.sub work correctly for Immutable while also
  //   making each state's tag unique, even if the other properties are the same. If there is no previous state, the
  //   current timestamp is used (so that even the first state in a chain is unique).
  // - ant is a claim in the protectedHeader, so that it is visible for use in merging without neededing to decrypt.
  //   Unlike antecedent, ant is the empty string for the initial state.
  // Alternatively, we could get rid of the antecedent in the payload, and jigger hashablePayload, subjectMatch, etc.
  ...
  store({...data}, {...options} = {}) {
    // data and options are copies, so we can bash them.
    options.time ||= Date.now();
    data.antecedent ||= options.time;
    options.ant ||= ((data.antecedent === options.time) ? '' : data.antecedent);
  ...
  async commonStateAndMessages(stateTags) {
    ...
    // - The remaining elements contains all and only those messages that are included in listOfStates after the common State of the first element returned.
    ...
    // However, if the first state in the list is near the tail of all the others, it will traverse that far through the others.
    ...
    let [candidateTag, ...otherStateTags] = stateTags;

    const candidateMessages = new Set(); // As we descend through the first state's candidates, keep track of past messages.
    // For each of the other states (as elements in two arrays):
    ...
    while (candidateTag) {
      // Check if this candidate is an ancestor of all other states
      const isCandidateInHistory = async (otherSeen, otherIndex) => { // Return true IFF the current candidate is in the given State's history.
	// As we go, we record the data seen for this other State.
	while (!otherSeen.has(candidateTag)) { // Fast check of what we've seen so far.
	  const otherTag = otherStateTags[otherIndex];     // Get the state, and return false if we've reached the end.
	  if (!otherTag) return false;                         // Otherwise, go one further level deeper in this state.
	  ...
	  otherStateTags[otherIndex] = verifiedState.protectedHeader.ant;
	}
	return true;  // We found a match!
      };
      const testResults = await Promise.all(othersSeen.map(isCandidateInHistory));
      if (testResults.every(Boolean)) { // We found a match in each of the other States.
	...
	return [candidateTag, ...candidateMessages.keys()];
      }
      ...
      candidateTag = verifiedState.protectedHeader.ant;
    } // end while

    return [];   // No common ancestor foudn
  }
  async forStates(verifiedState, callback) {
    while (verifiedState) {
      await callback(verifiedState);
      const ant = verifiedState.protectedHeader.ant;
      verifiedState = ant && await this.getVerified({tag: ant, member: null});
    }
  }
}
...
export class VersionedXCollection extends MutableCollection {
  ...
  async store({...data}, {tag, encryption, ...options} = {}) {
    // Hidden pun:
    ...
    const verifiedVersion = await this.getVerified({tag, member: null});
    if (verifiedVersion) data.antecedent = verifiedVersion.text; // data is a copy.
    const versionTag = await this.versions.store(data, {encryption, ...options});
    if (!versionTag) return '';
    // Default tag to versionTag, and specifiy no encryption.
    const versionedTag = await super.store(versionTag, {tag: tag || versionTag, encryption: '', ...options});
    return versionedTag;
  }
  async remove(tagOrOptions) {
    const {tag, ...options} = tagOrOptions.length ? {tag: tagOrOptions} : tagOrOptions;
    let verifiedVersion = await this.getVerified({tag, member: null});
    while (verifiedVersion) {
      await this.versions.remove({tag: verifiedVersion.text, ...options});
      const ant = verifiedVersion.protectedHeader.ant;
      verifiedVersion = ant && await this.getVerified({tag: ant, member: null});
    }
    return super.remove(tagOrOptions);
  }
  async retrieve(tagOrOptions) {
    const verifiedVersion = await this.getVerified(tagOrOptions);
    if (!verifiedVersion) return '';
    const versionTag = verifiedVersion.text;
    const verified = await this.versions.retrieve({tag: versionTag, ...(tagOrOptions.length ? {} : tagOrOptions)});
    this.log('retrieve', {tagOrOptions, verifiedVersion, versionTag, verified});
    if (verified) delete verified.json.antecedent; // fixme
    return verified;
  }
