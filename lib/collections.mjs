...
export class Collection extends EventTarget {
  ...
  constructor({name, services = [], preserveDeletions = !!services.length, persistenceClass = Collection.Persist, dbVersion = version, debug = false,
	      multiplex = true}) { // Causes synchronization to reuse connections for different Collections on the same service.
    super();
    Object.assign(this, {name, preserveDeletions, persistenceClass, dbVersion, multiplex, debug});
  ...
  async getVerified({tag, synchronize = true, ...options}) { // synchronize, get, and verify (but without decrypt)
    if (synchronize) await this.synchronize1(tag);
    const signature = await this.get(tag);
    if (!signature) return signature;
    return this.constructor.verify(signature, options);
  }
  ...
  mergeSignatures(tag, validation, signature) { // What shall be persisted? Usually just the signature.
    return signature;
  }
  ...
  async synchronize(...services) { // Start running the specified services (in addition to whatever is already running).
    ...
    for (let service of services) {
      ...
      await Synchronizer.create(this, service, {debug: this.debug}); // Reaches into our synchrnonizers map and sets itself immediately.
    }
  }
  ...
  async disconnect(...services) { // Shut down the specified services.
    ...
    for (let service of services) {
      ...
      if (!synchronizer) {
	console.log(`${this.name} does not have a service named '${service}' to disconnect.`);
	continue;
      }
  ...
  async ensureSynchronizer(serviceName, connection, dataChannel) { // Make sure dataChannel matches the synchronizer, creating Synchronizer only if missing.
    ...
    if (!synchronizer) {
      console.log('\n\n*** Synchronizer#ensureSynchronizer create matching synchronizer', serviceName);
      ...
      const connectionMatch = synchronizer.connection === connection;
      const collectionName = synchronizer.collectionName;
      const channelLabel = dataChannel.label;
      const promiseMatch = await synchronizer.dataChannelPromise === dataChannel;
      console.log({collectionName, channelLabel, connectionMatch, promiseMatch, dataChannel});
...
export class VersionCollection extends MutableCollection { // Needs to be exported so that that router.mjs can find it.
  async tagForWriting(tag, validation) { // Use tag if specified (e.g., put/delete during synchronization), othwerwise reflect both sub and antecedent.
    ...
    const use = tag || Credentials.encodeBase64url(await Credentials.hashText(validation.protectedHeader.ant + validation.signature));
    return use;
  }
...
export class VersionedCollection extends MutableCollection {
  ...
  async getVersions(tag) { // Promises the parsed timestamp => version dictionary IF it exists, else falsy.
    ...
    const combined = this.combineTimestamps(null, ...json);
    return combined;
  }
  ...
  async store(data, options = {}) { // Determine the antecedent, record it in the signature, and store that
	...
	// Of course, even if encrypted, they could also get this from live traffic analysis, so maybe encrypting it would juse
  ...
  async mergeSignatures(tag, validation, signature) { // Merge the new timestamps with the old.
    ...
    let previous = validation.existing?.json;
    if (!previous) return this.constructor.ensureString(signature);   // No previous, just use new signature.

    let next = validation.json;
    if (validation.protectedHeader.iat < validation.existing.protectedHeader.iat) { // Arrange for next and signature to be later one by signed timestamp.
      ...
      signature = validation.existing.signature;
      [previous, next] = [next, previous];
    }

    // Find the timestamps of previous whose VALUES that are not in next.
    let keysOfMissing = null;
    if (!Array.isArray(previous) && !Array.isArray(next)) { // No point in optimizing through missingKeys if that makes us combineTimestamps anyway.
      keysOfMissing = this.missingKeys(previous, next);
      if (!keysOfMissing.length) return this.constructor.ensureString(signature); // Previous is a subset of new signature.
    }

    let owner = validation.protectedHeader.iss || validation.protectedHeader.kid; // We can only merge if we are an owner.
    let isOwner = owner === (Credentials.owner || Credentials.author);
    let canSign = isOwner || await Credentials.sign('', owner).catch(() => false);
    ...
    if (!canSign) { // We don't have owner and cannot get it.
      ...
      return JSON.stringify(flatten(validation.existing.signature, validation.signature));
    }
    const merged = this.combineTimestamps(keysOfMissing, ...flatten(previous, next));
    const options = {tags: [Credentials.author]};
    const sig = await this.constructor.sign(merged, options);
    return sig;
  }
  missingKeys(previous, next) { // Answer a list of those keys from previous that are not in next.
    const nextValues = new Set(Object.values(next));
    return Object.keys(previous).filter(key => key !== 'latest' && !nextValues.has(previous[key]));
  }
  combineTimestamps(keysOfMissing, previous, next, ...rest) { // Return a merged dictionary of timestamp => hash, containing all of previous and next.
	...
	for (; (missingIndex < keysOfMissing.length) && ((missingTime = keysOfMissing[missingIndex]) < nextTime); missingIndex++) {
	  merged[missingTime] = previous[missingTime];
	}
      }

      if (missingTime === nextTime) { // Two different values at the exact same time. Extremely rare.
	console.warn(this.name, `Unusual matching timestamp case.`);
	nextTimes ||= Object.keys(next); // We didn't need this for our loop. Generate now if needed.
	const nextNextTime = Math.min(keysOfMissing[missingIndex + 1] || Infinity,
				      next[nextTimes.indexOf(nextTime) + 1] || Infinity);
	const insertTime = nextTime + (nextNextTime - nextTime) / 2;
	// We already put these in order with previous first.
	merged[nextTime] = previous[nextTime];
	merged[insertTime] = next[nextTime];

      } else { // No conflicts. Just add next.
	merged[nextTime] = next[nextTime];
      }
    }
    ...
    return rest.length ? this.combineTimestamps(undefined, merged, ...rest) : merged;
  }
