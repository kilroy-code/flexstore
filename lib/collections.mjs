...
export class Collection extends EventTarget {
  ...
  async validateForWriting(tag, signature, operationLabel, synchronizer, requireTag = false) {
    ...
    const existingVerified = verified.existing = antecedent && await this.getVerified({tag: antecedent, synchronize: false});
...
export class VersionedCollection extends MutableCollection {
  ...
  async put(tag, signature, synchronizer = null) {
    //console.log('** put', tag, !!synchronizer);
    await super.put(tag, signature, synchronizer);
  }
  ...
  async mergeSignatures(tag, validation, signature) { // Merge the new timestamps with the old.
    ...
    console.log('** merge', tag, previous, validation.json); 
    ...
    if (validation.protectedHeader.iat < validation.existing.protectedHeader.iat) { // Arrange for next and signature to be later one by signed timestamp.
      console.log('** switching order', tag);
    ...
    const keysOfMissing = Object.keys(previous).filter(key => key !== 'latest' && !nextValues.has(previous[key]));
    console.log('*** missing:', tag, keysOfMissing);
    if (!keysOfMissing.length) return Collection.ensureString(signature); // Previous is a subset of new signature.
    ...
    if (!canSign) { // We don't have owner and cannot get it.
      ...
      console.log('*** combining', tag, [validation.existing.signature, validation.signature]);
    ...
    const merged = {}; // placeholder value
    let missingIndex = 0, missingTime;
    for (const nextTime in next) {
      missingTime = 0;
      if (nextTime !== 'latest') { // Merge any remaining keysOfMissing that come strictly before nextTime:
	...
	const nextNextTime = Math.min(keysOfMissing[missingIndex + 1] || Infinity, next[next.indexOf(nextTime) + 1] || Infinity);
	const insertTime = nextTime + (nextNextTime - nextTime) / 2;
	const previousActorFirst = previous.protectedHeader.act < validation.protectedHeader.act;
	if (previousActorFirst) { // We could flip these if even. Any point?
	  merged[nextTime] = previous[nextTime];
	  merged[insertTime] = next[nextTime];
	} else {
	  merged[nextTime] = next[nextTime];
	  merged[insertTime] = previous[nextTime];
	}
    ...
    console.log(`*** Need to sign merge of ${this.name} ${tag}: ${JSON.stringify(previous, null, 2)} + ${JSON.stringify(next, null, 2)} => ${JSON.stringify(merged, null, 2)}.`);
    const sig = await Collection.sign(merged, options);
    return sig;
  }
