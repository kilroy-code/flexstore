...
export class StateCollection extends ImmutableCollection {
  ...
  async forEachState(tag, callback) { // await callback(verifiedState, tag) on the state chain specified by tag.
    // Stops iteration and returns the first truthy value from callback.
    while (tag) {
      const data = await this.getVerified({tag, member: null});
      if (!data) return null;
      const result = await callback(data, tag);
      if (result) return result;
      tag = data.protectedHeader.ant;
    }
    return null;
  }
}
...
export class VersionedXCollection extends MutableCollection {
  ...
  async store({...data}, tagOrOptions = {}) {
    ...
    const {tag, encryption, ...options} = tagOrOptions.length ? {tag:tagOrOptions} : tagOrOptions;
  ...
  async remove(tagOrOptions) {
    ...
    await this.forEachState(tag, (_, tag) => {
      this.versions.remove({tag, ...options});
    });
    return super.remove(tagOrOptions);
  }
  async retrieve(tagOrOptions) {
    let {tag, time, hash, ...options} = tagOrOptions.length ? {tag:tagOrOptions} : tagOrOptions;
    if (!hash && !time) {
      const verifiedVersion = await this.getVerified({tag, members: null, ...options});
      if (!verifiedVersion) return '';
      hash = verifiedVersion.text;
    }
    if (hash) {
      const verified = await this.versions.retrieve({tag: hash, ...options});
      if (verified) delete verified.json.antecedent; // fixme
      return verified;
    }
    time = parseFloat(time);
    const result = await this.forEachState(tag, verified => {
      return (verified.protectedHeader.iat <= time) ? verified : '';
    });
    if (result) delete result.json.antecedent; //fixmer
    return result;
  }
  ...
  async forEachState(tag, callback) {
    ...
    return await this.versions.forEachState(verified?.text, callback);
  }
