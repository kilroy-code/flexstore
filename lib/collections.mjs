...
export class Collection extends EventTarget {
  ...
  constructor({name, services = [], preserveDeletions = !!services.length, persistenceClass = Collection.Persist, dbVersion = version, debug}) {
    super();
    Object.assign(this, {name, preserveDeletions, persistenceClass, dbVersion, debug});
...
export class VersionedCollection extends MutableCollection {
  ...
  async getVersions(tag) { // Promises the parsed timestamp => version dictionary IF it exists, else falsy.
    ...
    const combined = this.combineTimestamps(...json);
    return combined;
  }
  ...
  async mergeSignatures(tag, validation, signature) { // Merge the new timestamps with the old.
    ...
    this.log('** merge', tag, previous, validation.json); 
    ...
    const keysOfMissing = this.missingKeys(previous, next);
    this.log('*** missing:', tag, keysOfMissing);
    ...
    if (!canSign) { // We don't have owner and cannot get it.
      ...
      this.log('*** combining', tag, [validation.existing.signature, validation.signature]);
      return JSON.stringify([validation.existing.signature, validation.signature]);
    }
    const merged = this.combineTimestamps(previous, next, keysOfMissing);
    const options = {tags: [Credentials.author]};
    this.log(`*** Need to sign merge of ${this.name} ${tag}: ${JSON.stringify(previous, null, 2)} + ${JSON.stringify(next, null, 2)} => ${JSON.stringify(merged, null, 2)}.`);
  ...
  combineTimestamps(previous, next, keysOfMissing = this.missingKeys(previous, next)) { // Return a merged dictionary of timestamp => hash, containing all of previous and next.
  ...
  static async verify(signature, options = {}) { // An array of unmerged signatures can be verified.
    let verified = await super.verify(signature, options);
    if (verified) return verified;
    if (!signature.startsWith('[')) return undefined;
    signature = JSON.parse(signature); // (maybeInflate looks for '{', not '['.)
