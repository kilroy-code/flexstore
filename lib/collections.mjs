...
class Collection extends EventTarget {
  ...
  async store(data, options = {}) {
    ...
    await Promise.all(this.synchronizers.values().map(synchronizer => synchronizer.push(tag, signature)));
    return tag;
  }
  ...
  async validateForWriting(tag, signature, operationLabel, requireTag = false) {
    ...
    const existingVerified = antecedent && await this.getVerified(antecedent, 'skipSync');
...
class VersionCollection extends MutableCollection {
  ...
  antecedent(validation) {
    const antecedent = validation.protectedHeader.antecedent;
    if (antecedent === validation.protectedHeader.iat) return ''; // First version.
    return antecedent;
  }
...
export class VersionedCollection extends MutableCollection {
  ...
  async store(data, options = {}) {
    let versions,
	{tag, ...signingOptions} = this._canonicalizeOptions(options),
	time = Date.now(),
	versionOptions = Object.assign({time}, signingOptions);
    if (tag) {
  ...
  async remove(options = {}) { // Add an empty verion or remove all versions, depending on this.preserveDeletions.
    ...
    if (this.preserveDeletions) { // Create a timestamp => version with an empty payload. Otherwise merging with earlier data will bring it back!
      ...
      const signature = await Collection.sign('', {sub: tag, ...signingOptions});
