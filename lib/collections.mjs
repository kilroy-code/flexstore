...
class Collection extends EventTarget {
  ...
  synchronizers = {};
  get services() {
    return Object.keys(this.synchronizers);
  }
  async synchronize(...services) { // Start running the specified services (in addition to whatever is already running).
    ...
    for (let service of services) {
      if (synchronizers[service]) continue;
      const synchronizer = new Synchronizer({peerName: service, collection: this});
      synchronizers[service] = synchronizer;
      //synchronizer.connectAndSynchronize();
    }
  }
  async disconnect(services = this.services.slice()) { // Shut down the specified services.
    const {synchronizers} = this;
    for (let service of services) {
      const synchronizer = synchronizers[service];
      if (!synchronizer) {
	console.warn(`${this.name} does not have a service named '${service} to disconnect.`);
	continue;
      }
      service.disconnect();
      delete synchronizer[service];
    }
  }
