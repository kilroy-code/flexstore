...
export class WebRTC {
  ...
  negotiationneeded() { // Something has changed locally (new stream, or network change), such that we have to start negotiation.
    console.log('WebRTC#negotiationnneeded', this.label, this.peer.versionId);
...
export class PromiseWebRTC extends WebRTC {
  ...
  dataChannelsCount = 0;
  noteChannel(channel) { // Bookkeep open channel and return it.
    console.log(this.label, 'add datachannel', channel.label, 'to count', this.dataChannelsCount);
    this.dataChannelsCount++;
    channel.addEventListener('close', event => { // Close whole connection when no more data channels or streams.
      console.log(this.label, 'remove datachannel', channel.label, 'from count', this.dataChannelsCount);
      if (0 < --this.dataChannelsCount) return;
      if (0 < this.peer.getSenders()) return;
      this.close();
    });
    return channel;
  }
  createDataChannel(label = "data", channelOptions = {}) { // Promise resolves when the channel is open (which will be after any needed negotiation).
    ...
    return new Promise(async resolve => {
      ...
      this.noteChannel(channel); // Noted even before opened.
  ...
  resetPeer() { // Reset a 'connected' property that promised to resolve when opened, and track incoming datachannels.
    ...
    this.connected = new Promise(resolve => { // this.connected is a promise that resolves when we are.
      this.peer.addEventListener('connectionstatechange', event => (this.peer.connectionState === 'connected') && resolve(true));
    });
    this.peer.addEventListener('datachannel', event => { // Resolve promise made with getDataChannelPromise().
      // Per spec, this does not fire for channels that OUR side creates.
      const channel = event.channel;
      const label = channel.label;
      const waiting = this.waitingChannels[label];
      this.noteChannel(channel); // Regardless of whether we are waiting.
...
export class SharedWebRTC extends PromiseWebRTC {
  ...
  static ensure({service, multiplex = true, ...rest}) {
    ...
    console.log('SharedWebRTC.ensure', {service, multiplex, uuid: connection.uuid});
    return connection;
  }
  channelId = 0;
  close() {
    this.channelId = 0;
    super.close();
    console.log('connection/peer close', this.label, this.dataChannelsCount, 'datachannel remaining');
  ...
  async ensureDataChannel(channelName, channelOptions = {}, signals = null) { // Return a promise for an open data channel on this connection.
    ...
    const negotiated = id > 0; // The 0th channel is not negotiated -- i.e., one side creates and the other waits passively.
    const options = {id, negotiated, ...channelOptions};
    const allowOtherSideToCreate = !negotiated && signals; // Only the 0th with signals waits passively.
    console.log('SharedWebRTC#ensureDataChannel', this.label, {channelName, channelOptions,  channelOptions, id, negotiated, options, allowOtherSideToCreate}, this.peer.connectionState);
    if (negotiated) {
      await this.connected;
      console.log('SharedWebRTC#ensureDataChannel', this.label,'has now been connected for',  channelName, 'Making new data channel');
    }
    const promise = allowOtherSideToCreate ?
	  this.getDataChannelPromise(channelName) :
	  this.createDataChannel(channelName, channelOptions);
    if (allowOtherSideToCreate) this.signals = signals;
