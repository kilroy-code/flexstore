...
export class SharedWebRTC extends PromiseWebRTC {
  ...
  static ensure({service, key = service, multiplex = true, ...rest}) {
    let connection = this.connections.get(key);
    console.log('fixme', key, multiplex, connection);
    if (!connection) {
      connection = new this({label: service, uuid: uuid4(), key, multiplex, ...rest});
      if (multiplex) this.connections.set(key, connection);
    }
    return connection;
  }
  ...
  close(removeConnection = true) {
    console.log('close and reset channelId', this.label, this.constructor.connections, removeConnection ? 'remove' : 'keep', 'connections');
    this.channelId = BASE_CHANNEL_ID;
    super.close();
    if (removeConnection) this.constructor.connections.delete(this.key);
  }
  async ensureDataChannel(channelName, channelOptions = {}, signals = null) { // Return a promise for an open data channel on this connection.
    ...
    console.log('ensureDataChannel', {channelName, channelId:id, multiplex: this.multiplex, negotiated, allowOtherSideToCreate, options},
		this,
