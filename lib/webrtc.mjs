...
export class WebRTC {
  ...
  negotiationneeded() { // Something has changed locally (new stream, or network change), such that we have to start negotiation.
    console.log('\n\n*** WebRTC#negotiationnneeded', this.label, this.peer.versionId);
...
export class PromiseWebRTC extends WebRTC {
  ...
  getDataChannelPromise() {
    return new Promise(resolve => { // Resolves to an open data channel.
      this.peer.addEventListener('datachannel', event => resolve(event.channel));
    });
  }
  resetPeer() {
    super.resetPeer();
...
export class SharedWebRTC extends PromiseWebRTC {
  ...
  static ensure({service, multiplex = true, ...rest}) {
    ...
    console.log({service, multiplex, uuid: connection.uuid});
    return connection;
  }
  channelId = 0;
  close() {
