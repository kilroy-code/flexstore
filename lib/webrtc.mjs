...
export class PromiseWebRTC extends WebRTC {
  ...
  noteChannel(channel, source, waiting) { // Bookkeep open channel and return it.
    ...
    console.log(`addchannel ${source} to ${this.dataChannels.size}: ${this.label} ${channel.id} ${channel.label} ${existing ? 'exists' : 'new'} ${waiting ? 'waiting' : 'unsolicited'}`);
    ...
    channel.addEventListener('close', event => { // Close whole connection when no more data channels or streams.
      console.log(`removechannel ${source} from ${this.reportChannels()}: ${this.label} ${channel.id} ${channel.label}`);
      ...
      this.close();
      console.log('closed connection', this.label, 'because of', channel.label, this.peer.connectionState);
    });
    return channel;
  }
  ...
  resetPeer() { // Reset a 'connected' property that promised to resolve when opened, and track incoming datachannels.
    ...
    this.peer.addEventListener('datachannel', event => { // Resolve promise made with getDataChannelPromise().
      ...
      console.log('peer datachannel', label, waiting ? 'waiting' : 'not waiting');
      // Emperically:
...
export class SharedWebRTC extends PromiseWebRTC {
  ...
  static ensure({service, multiplex = true, ...rest}) {
    if (service === 'a') {console.error('*** how here without throw?'); }
    let connection = this.connections.get(service);
    if (!connection) {
      connection = new this({label: service, uuid: uuid4(), multiplex, ...rest});
  ...
  close() {
    console.log('zz close', this.label, this.constructor.connections);
    this.channelId = BASE_CHANNEL_ID;
    super.close();
    this.constructor.connections.delete(this.label);
    console.log('zz closed', this.label, this.constructor.connections);
  }
  async ensureDataChannel(channelName, channelOptions = {}, signals = null) { // Return a promise for an open data channel on this connection.
    ...
    console.log('ensureDataChannel', {channelName, id, multiplex: this.multiplex, negotiated, allowOtherSideToCreate, options},
		this,
