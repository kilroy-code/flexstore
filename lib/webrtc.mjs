...
export class PromiseWebRTC extends WebRTC {
  ...
  reportChannels() { // Return a report string useful for debugging.
    ...
    const kv = entries.map(([k, v]) => `${k}:${v.label}`);
    return `${this.dataChannels.size}/${kv.join(', ')}`;
  }
  noteChannel(channel, source) { // Bookkeep open channel and return it.
    ...
    const id = channel.id === null ? 1 : channel.id;
    console.log(`addchannel ${source} to ${this.dataChannels.size}: ${this.label} ${channel.id} ${channel.label} ${this.dataChannels.get(id) ? 'exists' : 'new'}`);
    this.dataChannels.set(id, channel);
    console.log(`service ${this.label} now has ${this.reportChannels()}`);
    channel.addEventListener('close', event => { // Close whole connection when no more data channels or streams.
      console.log(`removechannel ${source} from ${this.reportChannels()}: ${this.label} ${id} ${channel.label}`);
      this.dataChannels.delete(id);
      // If there's nothing open, close the connection.
      if (this.dataChannels.size) return;
      if (this.peer.getSenders().length) return;
      console.log('close connection', this.label, 'because of', channel.label, this.peer.connectionState);
      this.close();
      setTimeout(() => console.log('closed connection', this.label, 'because of', channel.label, this.peer.connectionState), 100);
    });
    return channel;
  }
  createDataChannel(label = "data", channelOptions = {}) { // Promise resolves when the channel is open (which will be after any needed negotiation).
    ...
    return new Promise(async resolve => {
      // Emperically
      // with mutiplex=false:
      //   9 occurences from our side (with versioned last), id is null
      //console.log(`createDataChannel ${this.label} ${label} ${JSON.stringify(channelOptions)}`);
      let channel;
      try {
	channel = this.peer.createDataChannel(label, channelOptions);
      } catch (e) {
	console.error(`failure to create ${label} channel ${channelOptions.id}:`, e);
	return;
      }
      //console.log(`created channel ${channel.id} ${channel.label}.`);
      this.noteChannel(channel, 'explicit'); // Noted even before opened.
      channel.onopen = _ => {
	//console.log(`create channel ${channel.id} ${channel.label} is open.`);
	resolve(channel);
      };
    });
  }
  ...
  getDataChannelPromise(label = "data") { // Resolves to an open data channel.
    //console.log('getDataChannelPromise', this.label, label);
  ...
  resetPeer() { // Reset a 'connected' property that promised to resolve when opened, and track incoming datachannels.
    ...
    this.peer.addEventListener('datachannel', event => { // Resolve promise made with getDataChannelPromise().
      ...
      // Emperically:
      // with multiplex=false
      //    9 occurences. I think the first channel opened gets numbered 0, and the other 1.
      //      on one side: 2 id=1 waiting on on our side; 7 id=0 from other side
      //      on one side: 2 id=0 from other side; 7 id=1, wating on our side
      //console.log(`eventchannel ${channel.label} ${channel.id} is open, ${waiting ? 'ours' : 'theirs'}`);
      this.noteChannel(channel, 'datachannel event'); // Regardless of whether we are waiting.
...
// TODO:
// 1. If not multiplexed, we currently get two channels between full synchronizing peers, one in each direction. We want just one, by name.
// 2. multiplexed failing:
//    - user synchronizer is not being set in time to report protocol in script updateRelay

const BASE_CHANNEL_ID = 10;
export class SharedWebRTC extends PromiseWebRTC {
  static connections = new Map();
  static ensure({service, multiplex = true, ...rest}) {
    let connection = this.connections.get(service);
    if (!connection) {
      connection = new this({label: service, uuid: uuid4(), ...rest});
      //if (multiplex) this.connections.set(service, connection);
    }
    return connection;
  }
  ...
  close() {
    ...
    super.close();
    this.constructor.connections.delete(this.label);
  }
  async ensureDataChannel(channelName, channelOptions = {}, signals = null) { // Return a promise for an open data channel on this connection.
    ...
    const negotiated = hasStartedConnecting;
    const allowOtherSideToCreate = !negotiated && !!signals; // Only the 0th with signals waits passively.
    const useSignals = signals && !hasStartedConnecting;
    const options = negotiated ? {id, negotiated, ...channelOptions} : channelOptions;
    /*console.log('ensureDataChannel', {channelName, id, negotiated, allowOtherSideToCreate, options},
		negotiated ? 'waiting' : 'proceeding',
		this.dataChannels.get(id),
		Object.values(this.dataChannels).find(channel => channel.label === channelName)
	       );*/
    if (hasStartedConnecting) {
      await this.connected; // Before creating promise.
      //await new Promise(resolve => setTimeout(resolve, Math.random() * 200));
      //console.log(`after waiting, proceeding to ${allowOtherSideToCreate ? 'get promise for' : 'create'} ${id} ${channelName}`);
    }
    else if (useSignals) this.signals = signals;
