...
export class PersistIndexedDB {
  ...
  constructor({collection, collectionLabel = collection.fullLabel, dbName = storageName, dbVersion = storageVersion} = {}) {
    ...
    //   We open a different db for each combined collectionLabel on demand.
  ...
  delete(tag) { // Promise to really remove tag from collection. (Not just writing empty JWS payload.)
    return new Promise(resolve => {
