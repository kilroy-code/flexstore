...
export class Synchronizer {
  constructor({serviceName = 'direct', collection,
    ...
    connection ??= SharedWebRTC.ensure({service: serviceName, configuration: rtcConfiguration, multiplex, debug: debug});
    console.log('fixme construct Synchronizer', serviceName, 'collection/connection:', !!collection, connection, multiplex);
  ...
  async connectRendevous(url = this.connectionURL) { // Connect through a fixed-ip address rendevous to another peer doing the same.
    ...
    if (operation === 'reset') {
      console.log('connectRendevous: before close', this.connection.channelId, this.connection.peer.versionId);
      this.connection.close();
      console.log('connectRendevous: after close', this.connection.channelId, this.connection.peer.versionId);
