...
export class Synchronizer {
  constructor({serviceName = 'direct', collection, debug = collection.debug, minVersion = version, maxVersion = minVersion, uuid = undefined, rtcConfiguration = undefined}) {
    ...
    console.log({serviceName, collectionName, uuid, multiplex: collection.multiplex});
  ...
  static async create(collection, serviceName, options = {}) { // Receive pushed messages from the given service. get/put/delete when they come (with empty services list).
    ...
    let connected = connection.channelId;
    console.info(synchronizer.label, synchronizer.connection.peer.connectionState, serviceName);

    if (connected) {
      console.log(`\n\n*** Synchronizer.create ${serviceName} when already`, synchronizer.connection.peer.connectionState);
      // We already have a connection. Just open another data channel for our use.
      synchronizer.dataChannelPromise = connection.ensureDataChannel(synchronizer.collectionName);
	...
	connected = await synchronizer.connectRendevous();
      ...
      connected = await synchronizer.connectDirectTesting(serviceName); // Used in testing.
    } else {
    ...
    console.log('** Synchronizer.create now synchronizing', synchronizer.label);
  ...
  set dataChannelPromise(promise) { // Set up message and close handling.
	...
	// Subtle: In rendevous, we close the connection, but can re-use the synchronizer.
  ...
  async startConnection(signalMessages) { // Machinery for making a WebRTC connection to the peer:
    ...
    this.dataChannelPromise = connection.ensureDataChannel(this.collectionName, {}, signalMessages);
