...
export class Synchronizer {
  constructor({serviceName = 'direct', collection,
    ...
    connection ??= SharedWebRTC.ensure({serviceLabel, configuration: rtcConfiguration, multiplex, debug});
    ...
    Object.assign(this, {serviceName, label, collection, debug, minVersion, maxVersion, uuid, rtcConfiguration,
  ...
  async fetch(url, body = null) { // As JSON
    ...
    const request = await fetch(url, body ? {method, headers: {"Content-Type": "application/json"}, body: JSON.stringify(body)} : {method})
	  .catch(() => null);
    if (request?.ok) console.log('ok', url);
    else {
      console.error(request?.statusText || 'failed', url);
      return null;
    }
  ...
  async connectServer(url = this.connectionURL) { // Connect to a relay over http. Compare connectRendevous
    ...
    const signals = this.startConnection();
    const response = await this.fetch(url, await signals);
  ...
  async startSynchronization() { // Wait for all preliminaries, and start streaming our tags.
    ...
    console.log('negotiated version', this.label, version, ourTags);
    Object.assign(this, {
  ...
  async endTags() { // The peer has finished streamTags().
    this.log('received end of peer tags');
    await this.startedSynchronization;
  ...
  ensureSynchronizedTag(tag, theirHash = '', ourHashPromise = null) {
	...
	if (!theirHash || !ourHashPromise || (theirHash !== await ourHashPromise)) {
	  ...
	  if (!theirHash || theirData?.length) {
	    const isServer = (process.title === 'ki1r0yflex');
	    if (await this.collection.put(tag, theirData, this)) {
	      if (this.debug || isServer) {
		console.log(this.label, 'received/put', tag, 'their/our hash:', theirHash || 'missingTheirs', (await ourHashPromise) || 'missingOurs', theirData?.length);
		if (isServer) await dumpFile('after put', tag, 'ServerStorage');
	      }
	    } else {
	      if (this.debug || isServer) console.log(this.label, 'unable to put', tag);
	    }
	  }
	}
