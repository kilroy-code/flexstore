...
export class Synchronizer {
  constructor({serviceName = 'direct', collection,
	       channelName, uuid, rtcConfiguration, // Somplex default behavior for these. See code.
    ...
    // uuid uniquely identifies this particular synchronizer.
    ...
    const collectionName = `${collection.constructor.name}/${collection.name}`;
    ...
    const connection = SharedWebRTC.ensure({service: serviceName, configuration: rtcConfiguration, multiplex: collection.multiplex, debug: debug});
  ...
  static async create(collection, serviceName, options = {}) { // Receive pushed messages from the given service. get/put/delete when they come (with empty services list).
    ...
    const {hostRequestBase, uuid, connection} = synchronizer;
    let started = connection.hasStartedConnecting;
    console.info(synchronizer.label, synchronizer.connection.peer.connectionState, serviceName
		 ,connection.hasStartedConnecting ? 'started' : 'first'
		);
    if (started) {
      // We already have a connection. Just open another data channel for our use.
      //console.log('awaiting connection', synchronizer.label, synchronizer.connection.peer.connectionState, serviceName);
      //await connection.connected;
      //console.log('got connection', synchronizer.label, synchronizer.connection.peer.connectionState, serviceName);
      synchronizer.dataChannelPromise = connection.ensureDataChannel(synchronizer.channelName);
      await this.dataChannelPromise;
    } else if (synchronizer.connectionURL.includes('/signal')) { // Rendevous with another peer by exchanging signals.
	started = await synchronizer.connectRendevous();
    } else if (synchronizer.connectionURL.includes('/sync')) { // Connect with a server relay. (Signal and stay connected through sync.)
      started = await synchronizer.connectServer();
    } else if (serviceName === 'signals') { // Start connection and return the synchronizer. Must be continued with completeSignalsSynchronization();
      await synchronizer.startConnection();
      return synchronizer;
    } else if (Array.isArray(serviceName)) { // A list of "receiving" signals.
      await synchronizer.startConnection(serviceName);
      ...
      started = await synchronizer.connectDirectTesting(serviceName); // Used in testing.
    } else {
  ...
  async connectRendevous(url = this.connectionURL) { // Connect through a fixed-ip address rendevous to another peer doing the same.
    ...
    if (operation === 'reset') {
      console.log('before close', this.connection.channelId, this.connection.peer.versionId);
      this.connection.close();
      console.log('after close', this.connection.channelId, this.connection.peer.versionId);
      const ourAnswer = await this.startConnection(rest); // (Re-)Start our connection with response from the other peer.
      //console.log('ourAnswer:', this.label, ourAnswer);
      const secondRequest = await this.post(url, ourAnswer); // Tell them about our answer.
      //console.log('ignoring:', this.label, secondRequest);
