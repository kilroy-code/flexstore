...
export class Synchronizer {
  constructor({serviceName = 'direct', collection, debug, minVersion = version, maxVersion = minVersion, uuid = uuid4(), rtcConfiguration = undefined}) {
    ...
    const collectionName = this.collectionName = `${collection.constructor.name}/${collection.name}`;
    ...
    Object.assign(this, {serviceName, label, collectionName, collection, debug, minVersion, maxVersion, uuid, rtcConfiguration,
			 ...
			 connection: SharedWebRTC.ensure({label: this.label, configuration: this.rtcConfiguration, debug: this.debug}),
  ...
  static async create(collection, serviceName, options = {}) { // Receive pushed messages from the given service. get/put/delete when they come (with empty services list).
    ...
    console.info(synchronizer.label, connected ? 'connected' : 'connecting', serviceName);

    if (connected) {
      console.log(`\n\n*** creating ${serviceName} when already connected`, this);
