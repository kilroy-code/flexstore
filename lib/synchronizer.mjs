...
export class Synchronizer {
  ...
  static async create(collection, serviceInfo, options = {}) { // Receive pushed messages from the given service. get/put/delete when they come (with empty services list).
    const serviceName = (typeof(serviceInfo) === 'string') ? serviceInfo : collection.name;
    ...
    } else if (serviceInfo.synchronizers) { // Duck typing for passing a collection directly as the serviceInfo. Used in testing.
      connected = await synchronizer.connectDirectTesting(serviceInfo);
    } else {
      throw new Error(`Unrecognized service format: ${serviceInfo}.`);
    }
