...
export class Synchronizer {
  constructor({serviceName = 'direct', collection,
    ...
    channelName ??= collection.channelName || `${collection.constructor.name}/${collection?.name}`;
    const label = `${channelName}/${uuid}`;
  ...
  async connectChannel() {
    ...
    if (started) {
      ...
      started = await this.connectServer(); // Just like a sync
      ...
      const promisedSignals = this.startConnection([]); // Start our connection with response from the other peer.
      ...
      started = await promisedSignals;
      await this.fetch(url, started); // Tell the peer about our answer.
  ...
  async connectServer(url = this.connectionURL) { // Connect to a relay over http. Compare connectRendevous
    ...
    const response = await this.fetch(url, signals);
