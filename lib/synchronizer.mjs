...
export class Synchronizer {
  ...
  async fetch(url, body = null) { // As JSON
    ...
    if (request?.ok) console.log('ok', url);
      ...
      const log = await fs.readFile('test-server.log', 'utf8');
      console.log(log);
      return null;
    }
  ...
  cleanUpIfFinished() { // If we are not waiting for anything, we're done. Clean up.
    ...
    const nSynchronized = this.checkedTags.size; // The number that we have been transferred, but that may be larger than the number saved.
  ...
  ensureSynchronizedTag(tag, theirHash = '', ourHashPromise = null) {
	...
	if (!theirHash || !ourHashPromise || (theirHash !== await ourHashPromise)) {
	  ...
	  // If the hashes don't match, regardless of whether written. (If it was only those we 'put', above, then synchronizationPromise 
	  this.checkedTags.add(tag);  // would repeatedly request tags that have already been checked.)
	}
