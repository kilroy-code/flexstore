...
export class Synchronizer {
  constructor({serviceName = 'direct', collection, debug, minVersion = version, maxVersion = minVersion, uuid = uuid4()}) {
    ...
    Object.assign(this, {serviceName, collection, debug, minVersion, maxVersion, uuid, connectionStartTime: Date.now()});
    // TODO: Use conflict-free naming of both classes and collection names.
    const name = this.collectionName = `${this.collection.constructor.name}/${this.collection.name}`;
    this.label = `${name}/${this.uuid}`;
    this.connectionURL = `${this.serviceName}/${this.label}`; // Where we can request a data channel that pushes put/delete requests from others.
    // Not used yet, but could be used to GET resources over http instead of through the data channel.
    this.hostRequestBase = this.serviceName.startsWith?.('http') && `${this.serviceName.replace(/\/(sync|signal)/)}/${name}`;
  ...
  static async create(collection, serviceName, options = {}) { // Receive pushed messages from the given service. get/put/delete when they come (with empty services list).
    ...
    let connected = false;
    console.info(synchronizer.label, 'connecting', serviceName);

    if (synchronizer.connectionURL.includes('/signal')) { // Rendevous with another peer by exchanging signals.
	connected = await synchronizer.connectRendevous();
      ...
      await synchronizer.startConnection(serviceName, {}); // Expicitly no ice. LAN only.
  ...
  async startConnection(signalMessages, configuration) { // Machinery for making a WebRTC connection to the peer:
    ...
    const connection = this.connection = SharedWebRTC.ensure({label: this.label, configuration, debug: this.debug});
