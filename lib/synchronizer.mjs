...
export class Synchronizer {
  ...
  async connectRendevous(url) { // Connect through a fixed-ip address rendevous to another peer doing the same.
    ...
    this.log('connected', this.connection);
  }
  ...
  noteRequest(tag, theirHash = '', ourHashPromise = null) {
	...
	if (!theirHash || !ourHashPromise || (theirHash !== await ourHashPromise)) {
	  ...
	  if (!theirHash && theirData?.length) {
	    this.log('received', tag, 'their/our hash:', theirHash || 'missingTheirs', (await ourHashPromise) || 'missingOurs', theirData?.length);
	    await this.collection.put(tag, theirData, this);
	  }
