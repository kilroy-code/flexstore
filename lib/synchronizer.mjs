...
export class Synchronizer {
  constructor({peerName = 'test', collection, debug, minVersion = 1, maxVersion = 1, uuid = uuid4()}) {
    Object.assign(this, {peerName, collection, debug, minVersion, maxVersion, uuid});
    ...
    this.hostRequestBase = this.peerName.startsWith?.('http') && `${this.peerName}/${name}`;
    this.connectionURL = `${this.peerName}/requestDataChannel/${this.label}`; // Where we can request a data channel that pushes put/delete requests from others.
  }
  static async create(collection, serviceInfo, options = {}) { // Receive pushed messages from the given service. get/put/delete when they come (with empty services list).
    ...
    if (synchronizer.hostRequestBase) {
      if (synchronizer.hostRequestBase.includes('rendevous')) {
	const rendevousURL = `${synchronizer.peerName}/${synchronizer.uuid}`;
	connected = await synchronizer.connectRendevous(rendevousURL);
      } else {
    ...
    await synchronizer.dataChannelPromise;
    synchronizer.log('starting synchronization');
    if (connected) await synchronizer.startedSynchronization;
    synchronizer.log('started synchronization');
    return synchronizer;
  }
  log(...rest) {
  ...
  async send(method, ...params) { // Sends to the peer, over the data channel
    ...
    if (payload.length > 16e3) console.error(this.label, 'Unsupportable payload size', payload.length); //16e3 is not a typo. Being conservative in our warning.
  ...
  async connectServer(url = this.connectionURL) { // Connect to a relay over http. Compare connectRendevous
    ...
    // TODO: stringify in log, not in arguments.
    this.log('outgoing signals', JSON.stringify(signals, null, 2));
    const body = JSON.stringify(signals);
    const request = await fetch(url, {method: 'POST', body});
    const response = await request.text();
    this.log('incoming signals', JSON.stringify(signals, null, 2));
    return this.completeConnection(JSON.parse(response));
  }
  async connectRendevous(url) { // Connect through a fixed-ip address rendevous to another peer doing the same.
    this.log('connectRendevous', url);
    // TODO: error handling from requests.
    const ourFirstSignals = await this.startConnection();
    const body = JSON.stringify(ourFirstSignals);
    const request = await fetch(url, {method: 'POST', body});
    const response = await request.text();
    const responseSignals = JSON.parse(response);
    ...
    if (operation === 'reset') {
      ...
      const secondRequest = await fetch(url, {method: 'POST', body: JSON.stringify(ourAnswer)}); // Tell them about our answer.
  ...
  request(tag) { // Answer a promise the resolves with the data.
    ...
    if (hostRequestBase) {
      ...
      // and peerName should be something like "http://localhost:3000/flexstore"
