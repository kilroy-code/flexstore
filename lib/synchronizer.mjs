...
export class Synchronizer {
  constructor({serviceName = 'direct', collection,
    ...
    console.log('construct Synchronizer', {serviceName, collectionName, uuid, multiplex: collection.multiplex});
  ...
  static async create(collection, serviceName, options = {}) { // Receive pushed messages from the given service. get/put/delete when they come (with empty services list).
    ...
    if (connected) {
      console.log(`Synchronizer.create ${serviceName} with existing connection`, synchronizer.connection.peer.connectionState);
  ...
  set dataChannelPromise(promise) { // Set up message and close handling.
	...
	console.log('datachannel close event', this.label);
