...
export class Synchronizer {
  constructor({peerName = 'test', collection, debug, minVersion = 1, maxVersion = 1}) {
    ...
    // The label must match in the peer in 
  ...
  async connect(signalMessages) { // Creates a WebRTC connection to the peer.
    ...
    const onClose = event => { this.connection.close(); this.closed.resolve(); };
  ...
  versions(min, max) { // On receiving the versions supported by the the peer, resolve the version promise.
  ...
  async startSynchronization() { // Wait for all preliminaries, and start streaming our tags.
    ...
    Object.assign(this, {
      ...
      endOfKnownTags: false // Is the peer finished streaming?
    });
  ...
  async getHash(tag) { // Whole signature (NOT protectedHeader.sub of content).
    return this.computeHash(await this.collection.get(tag) || 'missing');
  }
  ...
  async get(tag) { // Respond to a peer's get() request by sending a put reponse with the data.
    this.send('put', tag, await this.collection.get(tag));
  }
  async put(tag, signature) { // receive the response to a get() request by resolving the corresponding promise.
    ...
    // Currently, the purpose of put here is as a response to get when request is going over webrt rather than fetch.
    // In this case, promise should be truthy. There could also be future uses in which put
    // is sent over for other reasons, but for now, if that happens, it just gets dropped on the floor.
    // else await this.collection.put(tag, signature);
  }
